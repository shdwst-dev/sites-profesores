    ...
    return (
        <div className="space-y-10 pb-20 selection:bg-rose-500/30">
            <header className="flex justify-between items-center mb-8">
                <div>
                    <h1 className="text-3xl font-black text-white">Editar Formatos y Documentos Sistemas</h1>
            ...
            <section className="bg-slate-800/50 p-8 rounded-3xl border border-white/5 space-y-6">
                <div className="flex items-center gap-4 border-b border-white/5 pb-4 mb-4">
                    <div className="p-3 bg-rose-600 rounded-xl"><Calendar size={20} className="text-white" /></div>
                    ...
                    <button
                        onClick={async () => {
                            ...
                            setEntregables(await getEntregables('Sistemas'));
                        }}
                        className="p-2 bg-rose-600 rounded-lg hover:bg-rose-500 transition-colors"
                    >
                        ...
                        <div key={item.id || idx} className="grid md:grid-cols-12 gap-4 items-end bg-slate-900/50 p-4 rounded-xl border border-white/5">
                            ...
                            <div className="md:col-span-1 flex justify-end gap-2">
                                ...
                                <button
                                    ...
                                    className="p-3 bg-rose-600 text-white rounded-xl hover:bg-rose-500 transition-colors disabled:opacity-50"
                                >
            ...
            <section className="bg-slate-800/50 p-8 rounded-3xl border border-white/5 space-y-6">
                <div className="flex items-center gap-4 border-b border-white/5 pb-4 mb-4">
                    <div className="p-3 bg-rose-600 rounded-xl"><BookOpen size={20} className="text-white" /></div>
                    <div className="flex-1">
                    ...
                    <button
                        onClick={async () => {
                            await createDocumentoDescarga({ title: 'Nuevo Documento', link: '#', icon: 'FileText', color: 'bg-rose-50 border-rose-100 hover:border-rose-300' });
                            setDescargas(await getDocumentosDescarga('Sistemas'));
                        }}
                        className="p-2 bg-rose-600 rounded-lg hover:bg-rose-500 transition-colors"
                    >
                        <Plus size={20} className="text-white" />
...
function Input({ label, value, onChange }: { label: string, value: string, onChange: (val: string) => void }) {
    return (
        <div className="w-full">
            ...
            <input
                type="text"
                className="w-full bg-slate-900 border border-white/10 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-rose-500 transition-colors"
                value={value}
