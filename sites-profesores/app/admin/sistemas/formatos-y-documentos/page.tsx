...
export default function AdminSistemasFormatosDocumentos() {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            if (activeSection === 'entregables') {
                ...
                else await createEntregable({ ...formData, department: 'Sistemas' });
            } else if (activeSection === 'descargas') {
                if (editingItem) await updateDocumentoDescarga(editingItem.id, formData);
                else await createDocumentoDescarga({ ...formData, department: 'Sistemas' });
            }
            await loadData();
    ...
    const renderTable = (
        ...
        const bgColors: Record<string, string> = { rose: 'bg-rose-600' };
        const textColors: Record<string, string> = { rose: 'text-rose-400' };
        
        const bgColor = bgColors[color] || 'bg-rose-600';
        const txtColor = textColors[color] || 'text-rose-400';
    ...
    return (
        <div className="space-y-10 pb-20 selection:bg-rose-500/30">
            ...
            <div className="space-y-8">
                {renderTable('entregables', <Calendar className="text-white" size={20} />, 'Entregables Sistemas', 'Tabla de fechas y entregas importantes', entregables, [
                                ...
                                <>
                                    <div className="grid grid-cols-2 gap-4">
                                        ...
                                        <div>
                                            ...
                                            <input required type="text" value={formData.deadline} onChange={e => setFormData({...formData, deadline: e.target.value})} className="w-full bg-slate-950 border border-white/10 rounded-xl px-4 py-3 text-white focus:border-rose-500 focus:ring-1 focus:ring-rose-500 transition-all text-sm font-medium" placeholder="Ej: Cierre: Fin de Semana 1" />
                                        </div>
                                    </div>
                                    <div>
                                ...
                                <>
                                    ...
                                    <div className="grid grid-cols-2 gap-4 pt-2">
                                        ...
                                        <div>
                                            ...
                                            <input type="text" value={formData.color || ''} onChange={e => setFormData({...formData, color: e.target.value})} className="w-full bg-slate-950 border border-white/10 rounded-xl px-4 py-3 text-white focus:border-rose-500 transition-all text-sm font-medium" placeholder="Ej: bg-rose-50 border-rose-100..." />
                                        </div>
                                    </div>
                                </>
                            )}
